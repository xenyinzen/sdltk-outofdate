...
{
    SDL_Style *newstyle = SDL_StyleNew();
    
    if(st->RadioButton)
    {
        SDL_StyleSetImage(newstyle,"SDL_RadioButton",0,SDL_SkinGetImage(skin,st->RadioButton->Normal));
        SDL_StyleSetImage(newstyle,"SDL_RadioButton",1,SDL_SkinGetImage(skin,st->RadioButton->Selected));
    }
    if(st->HorizontalScrollbar)
    {
        if(st->HorizontalScrollbar->Normal)
        {
            SDL_StyleSetImage(newstyle,"SDL_HorizontalScrollbar",0,SDL_SkinGetImage(skin,st->HorizontalScrollbar->Normal->Arrow1));
            SDL_StyleSetImage(newstyle,"SDL_HorizontalScrollbar",1,SDL_SkinGetImage(skin,st->HorizontalScrollbar->Normal->Arrow2));
            SDL_StyleSetImage(newstyle,"SDL_HorizontalScrollbar",2,SDL_SkinGetImage(skin,st->HorizontalScrollbar->Normal->Scrollbar));
            SDL_StyleSetImage(newstyle,"SDL_HorizontalScrollbar",3,SDL_SkinGetImage(skin,st->HorizontalScrollbar->Normal->Thumb));
        }
    }
    if(st->VerticalScrollbar)
    {
        if(st->HorizontalScrollbar->Normal)
        {
            SDL_StyleSetImage(newstyle,"SDL_VerticalScrollbar",0,SDL_SkinGetImage(skin,st->VerticalScrollbar->Normal->Arrow1));
            SDL_StyleSetImage(newstyle,"SDL_VerticalScrollbar",1,SDL_SkinGetImage(skin,st->VerticalScrollbar->Normal->Arrow2));
            SDL_StyleSetImage(newstyle,"SDL_VerticalScrollbar",2,SDL_SkinGetImage(skin,st->VerticalScrollbar->Normal->Scrollbar));
            SDL_StyleSetImage(newstyle,"SDL_VerticalScrollbar",3,SDL_SkinGetImage(skin,st->VerticalScrollbar->Normal->Thumb));
        }
    }
    SDL_StylePush(newstyle);
    return 1;
}
...
int SDL_SkinApplyWindow(SDL_Window *window,SDL_Skin *skin,SDL_SkinWindow *w)
{
    ...
    while(widget)
    {
        ...
        case SKINFRAME:
            ...
            SDL_WidgetShow(widget->widget);
            SDL_WindowSetFrame(window,widget->widget);
            break;
        case SKINPANEL:
            p=widget->w.panel;
        ...
        case SKINSCROLLBAR:
            ...
            SDL_ScrollbarSetImage(widget->widget,SCROLLBAR_IMAGE_NORMAL_THUMB,  SDL_SkinGetImage(skin,s->Style.Normal->Thumb));
            SDL_ScrollbarSetImage(widget->widget,SCROLLBAR_IMAGE_NORMAL_BAR,    SDL_SkinGetImage(skin,s->Style.Normal->Scrollbar));
            SDL_ScrollbarSetImage(widget->widget,SCROLLBAR_IMAGE_PRESSED_THUMB, SDL_SkinGetImage(skin,s->Style.Pressed->Thumb));
...
SDL_SkinWidget *SDL_SkinParseTable(xmlDocPtr doc,xmlNodePtr cur)
{
    ...
    if(key != NULL)
    {
        ...
        if(!strcmp(key,"single"))
            table->mode = TABLE_MODE_SINGLE_ROW;       /* Only one can be selected          */
        if(!strcmp(key,"multiple"))
            table->mode = TABLE_MODE_MULTIPLE_ROWS;    /* Multiple entries can be selected  */
    }
