...
static void SDL_TableAddSelected(SDL_Table *table,int row,int column);
            ...
            
            {
                ...
                for(field = 0; field < Table->Columns; field++)
                {
                    ...
                    if(image)
                    {
                        ...
                        SDL_ImageBlitToSurface(image,NULL,surface,&r);                           
                    }
                    if(string)
...
int SDL_TableEventHandler(SDL_Widget *widget,SDL_Event *event)
{
    ...
    case SDL_KEYDOWN:
        ...
        case SDLK_DOWN:
            ...
            {
                if(Table->SelectMode == TABLE_MODE_SINGLE_ROW)
                {
                    SDL_TableAddSelected(Table,Table->CurrentRow++,0);	  
                    SDL_WidgetRedrawEvent(widget);
                }
#if 0
    ...
    if(Event->button.button == 1)
    {
        if(Event->motion.y > Table->ButtonHeight)
        {
            ...
            if(Table->SelectMode > 0)
            {
                /* Add to selected */
                SDL_TableAddSelected(Table,Table->CurrentRow,0);
                SDL_SignalEmit(Widget,"select-row",(void*)Table);
                SDL_WidgetRedrawEvent(Widget);
            }
