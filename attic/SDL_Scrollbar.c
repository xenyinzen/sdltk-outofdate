...
int SDL_ScrollbarStyleSetImage(SDL_Style *style,int state,SDL_Image *image);
void SDL_ScrollbarStyleSizeInfo(int control,int state,SDL_Rect *dims,SDL_StyleImage* StyleImage);

#define SCROLLSIZE 16

SDL_WidgetType SDL_ScrollbarGetType()
{
    SDL_WidgetType scrollbar;
  
    scrollbar = SDL_WidgetTypeGet("SDL_Scrollbar");
    ...
    if(scrollbar == 0)
    {
        ...
        scrollbar=SDL_WidgetTypeRegister("SDL_Scrollbar",&func);
...
int clickedinside(int x,int y, int control,int state)
{
    SDL_Rect dims;
...
int SDL_ScrollbarStyleSetImage(SDL_Style *style,int state,SDL_Image *image)
{
    ...
    SDL_StyleWidget *widget = SDL_StyleWidgetEntry(style,"SDL_Scrollbar");
...
static void SDL_ScrollbarStyleDraw(SDL_Widget *widget,SDL_Surface *surface,SDL_Rect *Area,SDL_StyleImage *s)
{
    ...
    SDL_ImageBlitToSurface(style->Normal->Scrollbar,surface,&scrollbar);

    arrow1.x = 0;
    arrow1.y = 0;
    arrow1.w = style->Normal->Arrow1->surface->w;
    arrow1.h = style->Normal->Arrow1->surface->h;
    SDL_ImageBlitToSurface(style->Normal->Arrow1,surface,&arrow1);
    ...
    SDL_ImageBlitToSurface(style->Normal->Arrow2,surface,&arrow2);


    thumb.x = Scrollbar->PixelValue + arrow1.w;
    thumb.y = 0;
    thumb.w = widget->Rect.w * Scrollbar->NormalStepValue / Scrollbar->MaxValue;   /* Width is the button height */;
    thumb.h = style->Normal->Thumb->surface->h;
    SDL_ImageBlitToSurface(style->Normal->Thumb,surface,&thumb);
}

#if 0

int SDL_StyleImageVerticalScrollbarSetImage(SDL_Style *style,int state,SDL_Image *image)
{
...
void SDL_ScrollbarStyleSizeInfo(int control,int state,SDL_Rect *dims,SDL_StyleImage* StyleImage)
{
    SDL_ScrollbarStyle *Scrollbar = (SDL_ScrollbarStyle*)StyleImage;
    ...
    case STYLE_STATE_NORMAL:
        State = Scrollbar->Normal;
        break;
    case STYLE_STATE_PRESSED:
        State = Scrollbar->Pressed;
        break;
    case STYLE_STATE_HIGHLIGHTED:
        State = Scrollbar->Highlighted;
        break;
    case STYLE_STATE_DISABLED:
        State = Scrollbar->Disabled;
        break;
    }
