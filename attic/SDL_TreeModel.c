...
/* lazy population , prealloc only of root table */
SDL_Model *SDL_TreeModelNew(int rows,int columns)
{
    SDL_Model *model;
    SDL_TreeModelIndex    *index,*index2;
    ...
    for(i=0;i<rows;i++)
    {
        ...
        for(j=0;j<columns;j++)
        {
            index2 = SDL_TreeModelNewIndex(model);        
            SDL_TreeModelAddColumn(model,index,index2);
        }
