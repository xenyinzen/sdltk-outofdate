...
void SDL_WindowCreateWidgetSurface(SDL_Window *window,SDL_Widget *widget)
{
    if(widget->Rect.w + widget->Rect.x > window->Widget.Rect.w)
        widget->Rect.w  = window->Widget.Rect.w  - widget->Rect.x;

    if(widget->Rect.h + widget->Rect.y > window->Widget.Rect.h)
        widget->Rect.h  = window->Widget.Rect.h  - widget->Rect.y;
...
SDL_WidgetList *SDL_WindowRemoveWidget(SDL_Widget *widget)
{
    SDL_WidgetList *temp;
    SDL_WidgetList *prev;
	...
	prev=NULL;
	    ...
	    if(temp->Widget == widget)
		    ...
		    /* tangg temporarily modified */
                    if (prev==NULL) {
                    	SDL_WindowSetTopOfWidgetList( temp->Next );
                    }
                    else {
                    	prev->Next =temp->Next;
		    }	
...
/* Set the top widget of the widget list of the top visible (active) window */
SDL_WidgetList *SDL_WindowSetTopOfWidgetList( SDL_WidgetList *Item )
{
    SDL_Window     *Win;
    ...
    {
        if (Item == NULL) {
        	Win->FocusWidget = NULL;
        	Win->WidgetList = NULL;
        }
        else {
        	Win->FocusWidget = Item->Widget;
        	Win->WidgetList = Item;
        }
    }
}
...
void SDL_WindowRealize(SDL_Window *Window)
{
    ...
    while(Item)
    {
        ...
        if(Item->Widget->Surface == NULL)
        {
            SDL_WindowCreateWidgetSurface(Window,Item->Widget);
        }
...
SDL_Surface *SDL_GetSubSurface(SDL_Surface *surface,SDL_Widget *widget)
{
    ...
    if(widget->Rect.w > 0 && widget->Rect.h > 0)
    {
        subSurface = SDL_CreateRGBSurfaceFrom(pixels,widget->Rect.w,widget->Rect.h,mainvid->format->BitsPerPixel,mainvid->pitch,
                                              mainvid->format->Rmask,mainvid->format->Gmask,
                                              mainvid->format->Bmask,mainvid->format->Amask);
...
void SDL_WindowSetFrame(SDL_Window *Window,SDL_Widget *frame)
{
    Window->Frame=frame;
    frame->Surface = Window->surface;
    SDL_WidgetSetDims(frame,Window->Widget.Rect.x,Window->Widget.Rect.y,Window->Widget.Rect.w,Window->Widget.Rect.h);
